...
uint8_t g_location_control_count = 0;  //执行频率不需要那么高的用这个事件计数，用在中断中
uint8_t g_turn_angle_control_count = 0;//执行频率不需要那么高的用这个事件计数，用在中断中
uint8_t g_line_speed_control_count = 0;//执行频率不需要那么高的用这个事件计数，用在中断中
...
float line_pid_control(void)  
{
    ...
    //这里的实际值我给的是角度偏差的值，如果需要红外传感器巡线可以把传感器的值送进去
    g_yaw_err = yaw_err0();
