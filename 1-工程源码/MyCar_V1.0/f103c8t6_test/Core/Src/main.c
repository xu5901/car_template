...
int main(void)
{
  ...
  HAL_TIM_Base_Start_IT(&htim3); //开启定时器2 中断
  ...
  {
      /* 转一圈，34*13*4=1768个脉冲 */
//       sprintf((char *)oledstring,"motor1:%d",(short)__HAL_TIM_GET_COUNTER(&htim2));//显示脉冲数
//       OLED_ShowString(0,0,oledstring,8,1);  //显示在OLED上
      
      sprintf((char *)oledstring,"motor1:%d",read_pluse(&htim2));//显示脉冲数
      OLED_ShowString(0,0,oledstring,8,1);  //显示在OLED上
      sprintf((char *)oledstring,"motor2:%d",-read_pluse(&htim4));//显示脉冲数
